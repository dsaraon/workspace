	...
	public void start() {
		...
		try {
			...
			while(true){
				...
				SimpleHttpHandler newHandler = new SimpleHttpHandler(this.rootDirectory);
				newHandler.handle(clientSocket);

//				clientSocket.close();				
			}
			
		} catch (IOException e) {
			System.out.println("Exception when listening to the port");
			e.printStackTrace();
		}
	}
